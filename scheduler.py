...
NIGHT_LOVERS = {"לב", "איתי", "אלינור"}
DIVERSE_AGENTS = {"שני", "רונית", "לירון", "גיא", "מאי"}
...
LEV_ELINOR_PATTERN = {
    "A": {"לב": 4, "אלינור": 3, "רונית": 4},  # שבוע A
    "B": {"לב": 5, "אלינור": 2, "רונית": 3},  # שבוע B
}
# נציגים שמוחרגים מבדיקת חזרות (ביקשו לעבוד רק לילות/משמרת קבועה)
EXEMPT_FROM_REPEAT_CHECK = {"גיא", "לב", "סימה", "אלינור", "איתי", "שרית", "ריקי"}
...
def generate_schedule(
    ...
    def can_assign(name, day, shift):
        ...
        if day == "שבת" and shift == "בוקר" and schedule[name]["שישי"] == "לילה":
            return False
            # גיא - מקסימום 3 לילות בשבוע
        if name == "גיא" and shift == "לילה":
            total_nights = sum(1 for d in DAYS_ORDER if schedule[name][d] == "לילה")
            if total_nights >= 3:
                return False
